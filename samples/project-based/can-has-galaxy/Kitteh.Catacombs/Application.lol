...
OBTW
Kitteh Catacombs application proves the shared TerminalUi layout is not
Galaxy-specific. RUNCATACOMBS owns its crawler session and generic router,
then calls CATSCREEN before every EOF-safe prompt. Commands return message
YARNs while this application is the only rendering side effect. Imports are
function-local and the bottom-level RUN call intentionally depends only on
hoisted top-level functions, never file order.
TLDR
