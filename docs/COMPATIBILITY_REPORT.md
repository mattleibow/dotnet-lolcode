...
| Repository shared registrations | 150 | pinned lci + dotnet-lolcode | Exact stdout bytes except CRLF normalization; stdin/CWD supported |
| Generated EndToEnd shared fixtures | 125 | pinned lci + dotnet-lolcode | Portable inline behavioral programs |
| Hand-authored shared fixtures | 18 | pinned lci + dotnet-lolcode | Core format and error metadata coverage |
...
HAI/KTHXBYE programs in the migrated language categories. **125** have shared
fixtures. **33** stay in C# because they are error-only, custom-loop/SRS/object
behavior, or need a stronger managed assertion. CodeAnalysis inline programs
